...
metadata$geo_accession <- trimws(metadata$geo_accession)
...
pca_df <- data.frame(
  PC1 = pca_res$x[, 1],
  PC2 = pca_res$x[, 2],
  Obesity = factor(metadata2$biological_state, levels = c("Low", "Mean", "High")),
  Sex = metadata2$Sex,
...
p <- ggplot(
  ...
  theme_minimal() +
  ggtitle("PCA of RNA-seq Samples by Obesity Group")
